...
const SetAllowance = ({ createTransaction, goBack, currentGasPrice }) => {
	...
	const onUnlock = async () => {
		...
		try {
			...
			const gasEstimate = await aripoolStakeContract.estimate.approve(
				aripoolARIContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString())
			);
			const transaction = await aripoolStakeContract.approve(
				aripoolARIContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString()),
				{
			...
			if (transaction) {
				createTransaction({
					...
					status: 'pending',
					info: t('aripoolARI.locked.transaction'),
	...
	return (
		<>
			...
			<TitleContainer>
				<Logo src="/images/ARI.png" />
				<PageTitle>{t('aripoolARI.title')}</PageTitle>
				<PLarge>{t('aripoolARI.locked.subtitle')}</PLarge>
			</TitleContainer>
			<ButtonRow>

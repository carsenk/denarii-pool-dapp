...
const SetAllowance = ({ createTransaction, goBack, currentGasPrice }) => {
	...
	const onUnlock = async () => {
		...
		try {
			...
			const gasEstimate = await balpoolTokenContract.estimate.approve(
				balpoolContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString())
			);
			const transaction = await balpoolTokenContract.approve(
				balpoolContract.address,
				parseEther(TOKEN_ALLOWANCE_LIMIT.toString()),
				{
			...
			if (transaction) {
				createTransaction({
					...
					status: 'pending',
					info: t('balpool.locked.transaction'),
	...
	return (
		<>
			...
			<TitleContainer>
				<Logo src="/images/balancer.png" />
				<PageTitle>{t('balpool.title')}</PageTitle>
				<PLarge>{t('balpool.locked.subtitle')}</PLarge>
			</TitleContainer>
			<ButtonRow>

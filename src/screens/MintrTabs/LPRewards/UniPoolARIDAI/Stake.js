...
const Stake = ({ walletDetails, goBack }) => {
	...
	const { unipoolARIDAIContract } = snxJSConnector;
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			const [univ2Held, univ2Staked, rewards] = await Promise.all([
				uniswapV2ARIDAIContract.balanceOf(currentWallet),
				unipoolARIDAIContract.balanceOf(currentWallet),
				unipoolARIDAIContract.earned(currentWallet),
			]);
			setBalances({
	...
	useEffect(() => {
		...
		unipoolARIDAIContract.on('Staked', user => {
			if (user === currentWallet) {
				fetchData();
			}
		});

		unipoolARIDAIContract.on('Withdrawn', user => {
			if (user === currentWallet) {
				fetchData();
			}
		});

		unipoolARIDAIContract.on('RewardPaid', user => {
			if (user === currentWallet) {
				fetchData();
			}
		});
		...
		return () => {
			if (snxJSConnector.initialized) {
				unipoolARIDAIContract.removeAllListeners('Staked');
				unipoolARIDAIContract.removeAllListeners('Withdrawn');
				unipoolARIDAIContract.removeAllListeners('RewardPaid');
			}
		};
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [currentWallet]);
		...
		<Container>
			...
			<Navigation>
				...
				<ButtonTertiary
					as="a"
					target="_blank"
					href={`https://etherscan.io/address/${unipoolARIDAIContract.address}`}
				>
					{t('lpRewards.shared.buttons.goToContract')} ↗
				</ButtonTertiary>
			</Navigation>
			<PageTitle>{t('balpoolARIDAI.title')}</PageTitle>
			<PLarge>{t('balpoolARIDAI.unlocked.subtitle')}</PLarge>
			<BoxRow>
				<DataBox
